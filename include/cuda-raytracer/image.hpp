...
// A simple image abstraction that provides method to create file / write to a output image file (currently, only png
// files are supported). The write_to_file function is to be called after the cuda kernel writes to the output frame
// buffer.
class image_t
{
  public:
