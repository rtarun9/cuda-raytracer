...
uint32_t scene_t::add_material(const material::material_t &mat)
{
...
__device__ hit_details_t scene_t::ray_hit(const math::ray_t &ray) const
{
    ...
    // beneath the surface rather than on it. This will cause the ray to continuously intersect the surface of the
    // sphere, causing continous intersections results in false - shadowing. By setting min_t to be greater than 0, this
