    ...
    for (int a = -11; a < 11; ++a)
    {
        for (int b = -11; b < 11; ++b)
        {
            ...
            if ((center - math::float3(4.0f, 0.2f, 0.0f)).len() > 0.9f)
            {
                ...
                {
                    // Diffuse.
                    auto index = world.add_material(material::material_t().create_lambertain_diffuse(get_random_float3_in_range(0.0f, 1.0f)));
                ...
                {
                    // Metal.
                    world.add_material(material::material_t().create_metal(get_random_float3_in_range(0.5f, 1.0f), get_random_float_in_range(0.0f, 0.5f)));
                    auto sphere = scene::sphere_t(center, 0.2f,  world.get_current_mat_index());
                    world.add_sphere(sphere);
                }
                else
